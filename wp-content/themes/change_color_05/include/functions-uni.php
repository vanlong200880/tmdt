...
function my_loginredrect( $redirect_to, $request, $user ) {
    ...
    if( in_array('administrator', $user->roles) || in_array('contributor', $user->roles) || in_array('author', $user->roles)) {
      return admin_url();
    } else {
      return $current_link;
    }
...
function do_this_in_an_hour() {
  ...
  if(time() - $_SESSION["expried"] > 15*60*60){
      session_destroy();
      session_unset();
      ...
      $paged = (($published_posts % 500) == 0)? round($published_posts / 500): round($published_posts / 500) + 1;
      $query = array(
        'post_type' => 'post',
...
add_action( 'init','do_this_in_an_hour' );
